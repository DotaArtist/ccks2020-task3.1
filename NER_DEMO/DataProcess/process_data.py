...
from Public.path import path_data_dir,\
    path_data2_dir, path_msra_dir, path_renmin_dir, path_mydata_dir
import numpy as np
import os
import jieba
...
from DataProcess.ccks_preprocessing import ccks_preprocessing


def add_vocab_type(sentence):
    _out = [0] * len(sentence)
    sentence_seg = list(jieba.cut(sentence))

    start = 0
    for ix, _seg in enumerate(sentence_seg):
        _out[start: start + len(_seg)] = [ix] * len(_seg)
        start += len(_seg)
...
class DataProcess(object):
    ...
    def get_data(self, one_hot: bool = True) -> ([], [], [], []):
        ...
        path_test = os.path.join(self.base_dir, "dev.txt")
    ...
    def __new_bert_text_to_index(self, file_path: str):
        """
        bert的数据处理
        处理流程 所有句子开始添加 [CLS] 结束添加 [SEP]
        bert需要输入 ids和types所以需要两个同时输出
        由于我们句子都是单句的，所以所有types都填充0,修改为分词类型
        # TODO
        ...
        with open(file_path, 'r') as f:
            ...
            for line in f:
                if (line != '\n') and (line != '@\n'):
                    ...
                    line_data_ids = [self.cls_index] + line_data_ids + [self.sep_index]
                    line_data_types = add_vocab_type("".join(line_char))  # 自定义id
                    line_data_types = [0] + line_data_types + [127]  # 截断句子标识
                    line_label = [0] + line_label + [0]
